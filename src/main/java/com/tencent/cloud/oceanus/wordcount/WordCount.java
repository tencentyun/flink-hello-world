...
public class WordCount {
    ...
    public static void main(String[] args) throws Exception {
        ...
        DataStream<String> streamSource =
                env.addSource(new CountDownSource(countDownSeconds))
                        .name(
                                "Count down for "
                                        + (countDownSeconds <= 0 ? "infinite" : countDownSeconds)
                                        + " seconds");

        // 数据处理算子
        DataStream<Counter> dataStream =
                streamSource
        ...
        // 定义数据目的
        dataStream.addSink(new LoggingSink()).name("Logging Sink");
